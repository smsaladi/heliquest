...
draw_helical_wheel <- function(helix_seq, FactC = 0.05, FONT1 = 3, FONT2 = 5, CEXFT = 1, CEXTEXT = 0.8, 
    Ang = 3.83972435439, Mom = 0.046225448735, FlFH = 0, ANGT = 100, NBMIN = 18, NBM2 = 36, NBMAX = 54, 
    ...
    names(colorlookup) <- c("A", "C", "D", "E", "F", "G", "H", "I", "K", "L", "M", "N", "P", "Q", 
        "R", "S", "T", "V", "W", "Y")
    ...
    for (i in 1:length(data)) {
        ...
        if (Flag == 0) {
            ...
            if ((FlFH == 0) & (i == 1)) {
                xsv = x
                ysv = y
                plot(c(xsv, x), c(ysv, y), type = "l", xlim = Xlimit, xlab = "", ylab = "", xaxt = "n", 
                  yaxt = "n", ylim = Ylimit, cex = 0.7, lwd = 0.8)
                par(new = T)
            } else {
                plot(c(xsv, x), c(ysv, y), type = "l", xlim = Xlimit, xlab = "", ylab = "", xaxt = "n", 
                  yaxt = "n", ylim = Ylimit, cex = 0.7, lwd = 0.8)
    ...
    for (i in 1:length(data)) {
        ...
        if ((i > NBMIN) & (i <= NBM2)) {
            ...
            if (i > NBM2) {
                ...
                # x=x*1.47
                x = x * (1 + (FactC * tail[i - NBM2] + FactC * tail[i - NBMIN] + FactC * tail[i - 
                  NBMIN] + FactC * tail[i]))
                # y=y*1.47
                y = y * (1 + (FactC * tail[i - NBM2] + FactC * tail[i - NBMIN] + FactC * tail[i - 
                  NBMIN] + FactC * tail[i]))
            } else {
                if (FlFH == 0) {
                  newang = 270 - (Ang * (180/pi))
        ...
        if (i == 1) {
            symbols(x, y, circles = FactC * tail[i], fg = "black", bg = color[seq[i]], xlim = Xlimit, 
                ylim = Ylimit, xlab = "", ylab = "", xaxt = "n", yaxt = "n", inches = FALSE)
            par(new = T)
            plot(x, y, xlim = Xlimit, ylim = Ylimit, xlab = "", ylab = "", type = "p", xaxt = "n", 
                yaxt = "n", pch = data[i], cex = tail[i], lwd = 2, col = colorTxt[seq[i]])
            par(new = T)
            Delt = (FactC * tail[i])/5
            # modif pour mettre N et C a la place de 1 et fin
            plot(x + (sqrt(2)/2) * (FactC * tail[i]) + Delt, y - (sqrt(2)/2) * (FactC * tail[i]) - 
                Delt, xlim = Xlimit, ylim = Ylimit, xlab = "", ylab = "", type = "p", xaxt = "n", 
                yaxt = "n", pch = "N", col = "red", cex = 2, lwd = 1)
        } else {
            if (i <= NBMAX) {
                symbols(x, y, circles = FactC * tail[i], fg = "black", bg = color[seq[i]], xlim = Xlimit, 
                  ylim = Ylimit, xlab = "", ylab = "", xaxt = "n", yaxt = "n", inches = FALSE)
                par(new = T)
                plot(x, y, xlim = Xlimit, ylim = Ylimit, xlab = "", ylab = "", type = "p", xaxt = "n", 
                  yaxt = "n", pch = data[i], cex = tail[i], lwd = 1, col = colorTxt[seq[i]])
            }
        }
        # modif pour mettre N et C a la place de 1 et fin
        if ((i == length(data)) | (i == NBMAX)) {
            ...
            plot(x + (sqrt(2)/2) * (FactC * tail[i]) + Delt, y - (sqrt(2)/2) * (FactC * tail[i]) - 
                Delt, xlim = Xlimit, ylim = Ylimit, xlab = "", ylab = "", type = "p", xaxt = "n", 
                yaxt = "n", pch = "C", col = "red", cex = 2, lwd = 1)
        }
        if (i != length(data)) {
            par(new = T)
        }
    ...
    # if ((xM != 0.001) && (xM != -0.001) && (yM != 0.001) && (yM != -0.001)){ par(new=T)
    # arrows(0,0,xM,yM, cex=2, lwd=2) }
    par(new = F)
}

# pdf(filename = 'PlotHelix-1.pdf', width = 10, height = 10) test1
draw_helical_wheel("KPKPKPKPKPKPKPKPKPKPKPKPKPKPK")

# test2
draw_helical_wheel("ACDEFGHIKLMNPQRSTVWY", FactC = 0.05, FONT1 = 3, FONT2 = 5, CEXFT = 1, CEXTEXT = 0.8, 
    Ang = 5.58074390482, Mom = 0.122081976943, FlFH = 0, ANGT = 100, NBMIN = 18, NBM2 = 36, NBMAX = 54)
# dev.off()
