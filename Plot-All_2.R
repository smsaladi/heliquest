bitmap("/ipmc/site/heliquest/tmpAmpH/AmpH-5913150144/PlotHelix-1.jpeg", type = "jpeg", res=200 )
par(mfrow=c(1,1), pty='s')

par(mar=c(4, 4, 3, 3))

data<-c("A","C","D","E","F","G","H","I","K","L","M","N","P","Q","R","S","T","V","W","Y")
seq<-c(4,1,3,3,1,4,8,1,2,1,1,6,7,6,2,5,5,1,1,1)
tail<-c(3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3)
FactC=0.05
FONT1=3 
FONT2=5 
CEXFT=1 
...
for (i in 1:length(data)){
  ...
  if (i == 1) {
  	symbols(x,y,circles=FactC*tail[i], fg="black",bg=color[seq[i]],xlim=Xlimit, ylim=Ylimit, xlab="", ylab="", xaxt='n', yaxt='n',inches=FALSE)
	par(new=T)
	plot(x, y, xlim=Xlimit, ylim=Ylimit, xlab="", ylab="", type="p", xaxt='n', yaxt='n',pch=data[i], cex=tail[i], lwd=2, col=colorTxt[seq[i]])
        par(new=T)
	Delt=(FactC*tail[i])/5
	# modif pour mettre N et C a la place de 1 et fin
        plot(x+(sqrt(2)/2)*(FactC*tail[i])+Delt, y-(sqrt(2)/2)*(FactC*tail[i])-Delt, xlim=Xlimit, ylim=Ylimit, xlab="", ylab="", type="p", xaxt='n',yaxt='n',pch="N", col="red", cex=2, lwd=1)
      }
      else{
    	if (i <=NBMAX) {
    	symbols(x,y,circles=FactC*tail[i], fg="black",bg=color[seq[i]],xlim=Xlimit, ylim=Ylimit, xlab="", ylab="", xaxt='n', yaxt='n',inches=FALSE)
    	par(new=T)
    	plot(x, y, xlim=Xlimit, ylim=Ylimit, xlab="", ylab="", type="p", xaxt='n', yaxt='n',pch=data[i], cex=tail[i], lwd=1, col=colorTxt[seq[i]])
    	}
	}
# modif pour mettre N et C a la place de 1 et fin
  if ((i == length(data)) | (i == NBMAX)) {
  	par(new=T)
        plot(x+(sqrt(2)/2)*(FactC*tail[i])+Delt, y-(sqrt(2)/2)*(FactC*tail[i])-Delt, xlim=Xlimit, ylim=Ylimit, xlab="", ylab="", type="p", xaxt='n',yaxt='n',pch="C", col="red", cex=2, lwd=1)
  }
  if (i != length(data)){
    par(new=T)
  }
